...
QWEN_VL_VARIANTS = {"qwen2.5-vl-3b": "Qwen/Qwen2.5-VL-3B-Instruct","qwen2.5-vl-7b": "Qwen/Qwen2.5-VL-7B-Instruct","qwen2.5-vl-7b-awq": "Qwen/Qwen2.5-VL-7B-Instruct-AWQ",}
...
class QwenVL(BaseVLM):
    ...
    def load(self, cache_dir: Optional[str] = None) -> None:
        ...
        self._model.eval()
        self._processor = AutoProcessor.from_pretrained(self.hf_model_id,cache_dir=cache_dir,)
        self._loaded = True
    ...
    def generate(
        ...
        inputs = self._processor(text=[text_input],images=[pil_image],return_tensors="pt",).to(self.config.device)
    ...
    def export_onnx(self, save_path: str, opset: int = 17) -> str:
        ...
        Uses the ONNX-safe Resize decorator pattern developed in kornia PR #3463 (torch.onnx.is_in_onnx_export() guard) to avoid shape-logic errors during
        tracing."""
        if not self._loaded:
            raise RuntimeError("Call load() first.")
